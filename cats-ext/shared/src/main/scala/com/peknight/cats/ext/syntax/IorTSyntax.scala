...
trait IorTSyntax:
  extension [A, B] (ior: Ior[A, B])
    def eLiftIT[F[_]: Applicative]: IorT[F, A, B] = IorT(ior.pure[F])
  end extension
  extension [A] (a: A)
    def lLiftIT[F[_] : Applicative, R]: IorT[F, A, R] = a.leftIor[R].eLiftIT[F]
    def rLiftIT[F[_]: Applicative, L]: IorT[F, L, A] = a.rightIor[L].eLiftIT[F]
  end extension
  extension [F[_], A] (fa: F[A])
    ...
    def feLiftIT[E](using ApplicativeError[F, E]): IorT[F, E, A] = IorT(fa.attempt.map(_.toIor))
  end extension
